...
![Stars Analyze tab (v1.7.4) — target path, identity, resolution mode, SMB share fields, and the two action buttons "Analyze" and "Who has access?"](docs/screenshots/stars-analyze-tab.png)
...
```powershell
$exe = "Stars-v1.7.4-Setup.exe"  # adapt to your version
$expected = (Get-Content "$exe.sha256").Split("  ")[0]
$actual   = (Get-FileHash $exe -Algorithm SHA256).Hash.ToLower()
if ($actual -eq $expected) { "OK — file matches" } else { "MISMATCH — do NOT use" }
```

On WSL / Linux / macOS, `sha256sum -c Stars-v1.7.4-Setup.exe.sha256` works directly.
